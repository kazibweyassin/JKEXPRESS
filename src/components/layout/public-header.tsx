      ...
      <div className="mx-auto flex h-[4.25rem] max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-0.5 xl:flex">
          {nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-md px-2.5 py-2 text-sm font-medium text-slate-600 transition hover:bg-navy-50 hover:text-navy-900"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="hidden items-center gap-2 lg:flex">
          <Button variant="default" size="sm" asChild>
            <Link href="/book-viewing">Book viewing</Link>
          </Button>
        ...
        <div className="space-y-1 px-4 py-3">
          {nav.map((item) => (
            <Link
              ...
              className="block rounded-md px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-navy-50 hover:text-navy-900"
            >
              {item.label}
            </Link>
          ))}
          <div className="flex flex-col gap-2 border-t border-slate-100 pt-3">
            <Button variant="default" asChild>
              <Link href="/book-viewing" onClick={() => setOpen(false)}>
                Book viewing
              </Link>
            </Button>
